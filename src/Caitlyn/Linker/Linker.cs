namespace Caitlyn
{
    ...
    public class Linker
    {
        ...
        /// <param name="targetProject">The targe
        /// <exception cref="ArgumentNullException">The <paramref name="source"/> is <c>null</c>.</exception>
        /// <exception cref="ArgumentNullException">The <paramref name="targetProject"/> is <c>null</c>.</exception>t project.</param>
        ...
        private ProjectItems GetTargetProjectItems(ProjectItems source, Project targetProject)
        {
            ...
            for (int i = 0; i < splittedFolder.Length; i++)
            {
                ...
                var folder = (from targetProjectItem in targetProjectItems.Cast<ProjectItem>()
                              where string.Equals(targetProjectItem.Name, sourceItemName, StringComparison.InvariantCultureIgnoreCase) &&
                                    targetProjectItem.IsFolder()
                              select targetProjectItem).FirstOrDefault();
        ...
        private void SynchronizeProjectItem(ProjectItems source, ProjectItems target, ProjectType targetProjectType,
            bool recursive, IEnumerable<string> fileFilter)
        {
            Argument.IsNotNull("source", source);
        ...
        /// <exception cref="ArgumentNullException">The <paramref name="source"/> is <c>null</c>.</exception>
        /// <exception cref="ArgumentNullException">The <paramref name="target"/> is <c>null</c>.</exception>
        private void AddFilesAndFolders(ProjectItems source, ProjectItems target, ProjectType targetProjectType,
            int levels, IEnumerable<string> fileFilter)
        {
            Argument.IsNotNull("source", source);
            Argument.IsNotNull("target", target);
        ...
        /// <exception cref="ArgumentNullException">The <paramref name="source"/> is <c>null</c>.</exception>
        /// <exception cref="ArgumentNullException">The <paramref name="target"/> is <c>null</c>.</exception>
        private void RemoveFilesAndFolders(ProjectItems source, ProjectItems target, ProjectType targetProjectType,
            int levels, IEnumerable<string> fileFilter)
        {
            Argument.IsNotNull("source", source);
            Argument.IsNotNull("target", target);
            ...
            for (int i = 1; i < target.Count + 1; i++)
            {
                ...
                var existingSourceItem = (from sourceItem in source.Cast<ProjectItem>()
                                          where string.Equals(targetItem.Name, sourceItem.Name, StringComparison.InvariantCultureIgnoreCase)
